...
/** Update */
exports.updateServer = function(column, value, guildid) {
    const core = require('./../../core/core');
    var con = core.con;
    console.log("Loading servers...");
    var sql = `UPDATE servers SET ${column} = '${value}' WHERE guildid = ${guildid};`;
    con.query(sql, function(err, result) {
        if (err) {
            return new Error(err);
        } 
        console.log(`Updated ${column} = ${value} for the guild ${guildid}.`);
        return;
    });
}
